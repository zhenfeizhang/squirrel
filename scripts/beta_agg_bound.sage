# credit: https://github.com/pq-crystals/security-estimates/blob/master/proba_util.py
...
### assuming signatures are crafted

alpha = 20
rho = 8192

dist = {-1: 1/2, 1: 1/2}    
dist23 = iter_law_convolution(dist, alpha * rho)
...
logq = 18
tau = 21
target_single_coeff_bound = 2^-128/n/logq/(2*tau+1)
while single_coeff_prob < target_single_coeff_bound:
    single_coeff_prob = tail_probability(dist23, )
    index -= 1
...
single_coeff_prob = tail_probability(dist23, 8192)

print(tau, RR(log(n*logq*(2*tau+1)*single_coeff_prob, 2)))

single_coeff_prob = 0
tau = 24
target_single_coeff_bound = 2^-128/n/logq/(2*tau+1)
while single_coeff_prob < target_single_coeff_bound:
    single_coeff_prob = tail_probability(dist23, index)
    index -= 1
